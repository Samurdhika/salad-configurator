...
import { calculateTotalWeight, calculateTotalPrice } from "../utils/calculations.ts";
  ...
  return (
    <div className="bg-zinc-800 rounded-[3rem] p-8 text-white w-full flex flex-col md:flex-row gap-8 shadow-xl">
      <div className="flex-1 bg-[#4a4a4a] rounded-3xl p-6 flex flex-col justify-between min-h-[180px]">
        <div>
          <p className="text-lg font-semibold mb-2">Valitut ainesosat ({activeIngredients.length})</p>
          <div className="flex flex-wrap gap-2">
            {activeIngredients.map((item) => (
              <div key={item.id} className="flex items-center gap-2 bg-zinc-700 px-3 py-1 rounded-full text-sm">
                <span>{item.name}</span>
                <button onClick={() => removeIngredient(item.id)} className="text-red-400 hover:text-red-600 font-bold">✕</button>
              </div>
            ))}
          </div>
        </div>
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="bg-white text-black font-bold text-lg py-2 px-6 rounded-full border-4 border-lime-400 shadow-md min-w-[140px] text-center">
        {value}
      </div>
